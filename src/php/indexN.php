...
if (isset($_REQUEST["BEHEER"])) {         // stap 2 vanaf TA
    if (empty($_SESSION['aantal'])) {
        header("Location: https://www.toernooiklapper.nl/inc-ng1/proef");
        exit;
    } else {
        header("Location: https://www.toernooiklapper.nl/inc-ng1/deincasso");
        exit;
    }
}
...
//    testMail(__LINE__ . nl2br(print_r($gegevens,true)));
    ...
    if (!empty($gegevens["herhaalbaar"])) {
        ...
        $herh = "<li><small>Doorlopende incasso is aangevinkt.</small></li>";
    }
...
// add mandate
        $mandaat = $_SESSION['plaatsbewijs'].$txarr[11];
        $mandatum = (getdate()['year'] - 1)."-09-01";
        $transaction->setMandate($mandaat, $mandatum, true);  // niet meer nodig?????
// add transaction
...
//print $sepaxml;
    ...
    $totaalbedragS = money_format('%.2n',$totaalbedrag);
    $fname = $_SESSION['fname'];
    $body = <<<TAG
<body style="background-color: LightSkyBlue">
<h4>$toernooinaam</h4>
<p>Het gegenereerde bestand is hier te downloaden: <a href='https://www.toernooiklapper.nl/inc-ng1/$fname' download>incassobestand</a> (<small>klik met de  rechtermuisknop!</small>)
</p>
...
<p>Het aantal transacties = $aantal, het totale bedrag € $totaalbedragS.<br>
TAG;
    if (!empty($fouten[0])) {
...
//    testMail(__LINE__ . nl2br(print_r($gegevens,true)));
    ...
    if (!empty($gegevens["herhaalbaar"])) {
        $creator->setIsFolgelastschrift();
        $herh = "<li><small>Doorlopende incasso is aangevinkt (test)</small></li>";
    }
...
// add mandate
    $mandaat = $_SESSION['plaatsbewijs'];
    $mandatum = (getdate()['year'] - 1)."-09-01";
    $transaction->setMandate($mandaat, $mandatum, true);
// add transaction
    $creator->addTransaction($transaction);
// generate the transfer file
$sepaxml = $creator->generateSepaDirectDebitXml();
...
<body style="background-color: LightSkyBlue">
<h4>$toernooinaam</h4>
<p>Het gemaakte bestand is hier te downloaden: <a href='https://www.toernooiklapper.nl/inc-ng1/$fname' download>incassobestand</a> (<small>klik met de  rechtermuisknop!</small>)</p>
<p>Sla dit bestand op in uw PC en voer het binnen drie dagen in bij uw online
bankieren!</p>
